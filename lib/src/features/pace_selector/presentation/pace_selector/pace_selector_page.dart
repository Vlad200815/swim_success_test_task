...
class PaceSelectorPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<PaceSelectorCubit, PaceSelectorState>(
      listenWhen: (previous, current) =>
          previous.submissionStatus != current.submissionStatus,
      listener: (context, state) {
        if (state.submissionStatus == PaceSubmissionStatus.failure) {
        ...
        if (state.submissionStatus == PaceSubmissionStatus.success) {
          UserListPage.open(context);
        }
      },
      child: Scaffold(
        body: SafeArea(
            ...
            child: Column(
              ...
              children: [
                ...
                BlocBuilder<PaceSelectorCubit, PaceSelectorState>(
                  builder: (context, state) {
                    final isLoading =
                        state.submissionStatus == PaceSubmissionStatus.loading;

                    return ElevatedButton(
                      onPressed: isLoading
                          ? null // disabled while in flight — no double-submit
                          : () => context
                                .read<PaceSelectorCubit>()
                                .postPaceSeconds(),
  ...
  static GoRoute route() {
    return Navigation.getRoute(name, path, (state) {
      return BlocProvider(
        create: (_) => PaceSelectorCubit(postPaceSecondsUseCase: di.get()),
